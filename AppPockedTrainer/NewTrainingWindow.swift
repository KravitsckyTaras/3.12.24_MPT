    ...
    var body: some View {
        VStack {
            ...
            .sheet(isPresented: $showingDaysOfWeek) {
                DaysOfWeekView(inputFields: $inputFields, showingDaysOfWeek: $showingDaysOfWeek)
            }
            .sheet(isPresented: $showingDownloadDays) {
                DownloadDaysView(inputFields: $inputFields, showingDownloadDays: $showingDownloadDays)
            }
            ...
            HStack {
                ...
                Button(action: {
                    while true {
                        // Проверяем, больше ли одного элемента в массиве inputFields
...
struct NewTrainingWindow_Previews: PreviewProvider {
